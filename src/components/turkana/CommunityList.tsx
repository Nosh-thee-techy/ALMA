// CommunityList: monitored downstream communities with propagation timing.
import { useState } from "react";
import { MapPin, Users, Clock, Radio } from "lucide-react";
import { communities, tierMeta, type Community } from "@/lib/turkana-data";
      ...
      <div className="rounded-lg border border-border bg-card">
        <div className="border-b border-border px-5 py-4">
          ...
          <p className="text-xs text-muted-foreground">
            {communities.length} points along the Omo River and Lake Turkana shoreline
          </p>
        </div>
...
function PropRow({ label, value, tone }: { label: string; value: string; tone: "watch" | "warning" | "severe" }) {
  const toneCls =
    tone === "severe"
  ...
  return (
    <div className="flex items-center justify-between rounded px-3 py-2 text-sm" style={{}}>
      <span>{label}</span>
